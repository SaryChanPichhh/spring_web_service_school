...
base_dir = r"E:\SETEC INSTITUTE\Semester IIV\Java\web_service\src\main\java\com\example\web_service\seeder"
os.makedirs(base_dir, exist_ok=True)
seeder_file = os.path.join(base_dir, "DatabaseSeeder.java")
...
public class DatabaseSeeder implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        if (categoryRepository.count() == 0) {
            ...
            category.setTitle("Fast Food"); // FIXED
        ...
        if (deliveryRepository.count() == 0) {
            ...
            delivery.setUserId(user.getId()); // FIXED: setUserId instead of setUser
            delivery.setName("Speedy Delivery");
            delivery.setPhone("0987654321");
            delivery.setStatus(true); // FIXED: status is boolean
        ...
        if (menuRepository.count() == 0) {
            menu = new Menu();
            menu.setResId(restaurant.getResId()); // FIXED: using resId instead of setRestaurants
            menu.setCateId(category.getId().intValue()); // FIXED: using cateId instead of setCategories
            menu.setName("Whopper");
            menu.setPrice(5.99);
            menu.setRating(4); // Menu rating might be Integer or Double, wait I will assume Integer
        ...
        if (reviewRepository.count() == 0) {
            ...
            review.setUser(user);
            review.setResId(restaurant.getResId()); // FIXED
            review.setRating(5); // FIXED
        ...
        if (favoritesRepository.count() == 0) {
            ...
            favorites.setUserId(user.getId()); // Might need setUserId instead of setUser
            favorites.setResId(restaurant.getResId()); // Might need setResId
            favorites.setStatus(true); // FIXED
        ...
        if (saleHeaderRepository.count() == 0) {
            ...
            saleHeader.setUserId(user.getId()); // Might need setUserId
            saleHeader.setDeliveryId(delivery.getId().intValue());
            saleHeader.setExchangeRateId(exchangeRate.getId().intValue()); // FIXED
            saleHeader.setTotal(5.99);
            saleHeader.setStatus(true); // FIXED
        ...
        if (saleDetailRepository.count() == 0) {
            ...
            saleDetail.setHeaderId(saleHeader.getId()); // FIXED
            saleDetail.setCouponId(coupon.getCouponId().intValue()); // FIXED
            saleDetail.setResId(restaurant.getResId()); // FIXED
            saleDetail.setQty(1);
        ...
        if (couponAssignmentRepository.count() == 0) {
            ...
            ca.setCouponId(coupon.getCouponId().intValue()); // FIXED
            ca.setRestaurantId(restaurant.getResId()); // FIXED
            ca.setMenuId(menu.getId()); // FIXED
        ...
        if (freeDeliveryAssignmentRepository.count() == 0) {
            ...
            fda.setRestaurantId(restaurant.getResId()); // FIXED
            fda.setMenuId(menu.getId()); // FIXED
...
print(f"Created updated seeder at {seeder_file}")
